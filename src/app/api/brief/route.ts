...
const BodySchema = z.object({
  ...
  hackathon_guidelines_url: z.string().url().nullable().default(null),
})
...
export async function POST(req: Request) {
  ...
  const { session_id, project_context, hackathon_context, pitch_deck_gcs, notes_gcs, hackathon_guidelines_url } = parsed.data
  ...
  const { data: hackathonBrief, error: hbError } = await supabase
    ...
    .insert({
      ...
      guidelines_url: hackathon_guidelines_url,
      status: 'extracting',
      is_active: false,
    })
    .select('id')
  ...
  const briefQueue = process.env.CLOUD_TASKS_BRIEF_QUEUE ?? process.env.CLOUD_TASKS_QUEUE_NAME

  if (devMode || !briefQueue) {
    ...
    const baseUrl  = process.env.TASK_AUDIENCE ?? `https://${req.headers.get('host')}`
    const taskSa   = process.env.CLOUD_TASKS_SERVICE_ACCOUNT ?? `debrief-demo-room-live@${project}.iam.gserviceaccount.com`
    const targetUrl = `${baseUrl}/api/tasks/brief-extraction`
